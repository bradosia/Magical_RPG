...
#include "cActor.h" // Character, Enemies, Projectiles, Tokens
#include "cModel.h"
#include "cTexture.h"
#include "cLogicExtExt.h"
...
private:
	...
	std::unordered_map<std::string, std::function<void(cLogic*)>> CBmap;
...
public:
	...
	cActor* getActorMain();
	void on(std::string event, std::function<void(cLogic*)> CB);
};
