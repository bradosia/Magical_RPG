...
bool cMonster2::Attack(std::list<cBicho*> &players,
		std::list<cProjectile*> &projectiles, bool *proj_collision_map) {
	...
	if (GetFrame() == 6 && GetDelay() == 0) {
		...
		if (GetState() == STATE_ATTACKUP) {
			x = GetX()
					+ (GetBaseWidth() - projectiles.back()->GetHitWidth()) / 2;
			...
			if (projectiles.back()->ComputeEnemyCollisions(players)
					|| projectiles.back()->CollidesObjUD(proj_collision_map,
							false)
					|| projectiles.back()->CollidesObjUD(proj_collision_map,
							true))
			...
			x = GetX();
			y = GetY()
					+ (GetBaseHeight() - projectiles.back()->GetHitHeight())
							/ 2;
			projectiles.back()->SetState(STATE_LEVITATELEFT);
			projectiles.back()->SetPosition(x, y);
			//evaluo si el proyectil aparece encima de un enemigo o objeto
			if (projectiles.back()->ComputeEnemyCollisions(players)
					|| projectiles.back()->CollidesObjLR(proj_collision_map,
							false))
...
bool cMonster2::IA(bool *collision_map, bool *proj_collision_map,
		std::list<cBicho*> &enemies, cBicho* p1) {
	int i;
	...
	//if any player is reachable, I assign the corresponding attack state
	if (hitbox_p1.right > arrow_x
			&& hitbox_p1.left < arrow_x + arrow.GetHitWidth()) {
		if (hitbox_p1.bottom > GetY()) {
			for (i = ((GetY() + GetBaseHeight()) / TILE_SIZE);
					i <= (hitbox_p1.bottom / TILE_SIZE) && !collision; i++) {
				collision = proj_collision_map[(GetX() / TILE_SIZE)
						+ i * SCENE_WIDTH];
			}
			if (!collision) {
			...
			for (i = (hitbox_p1.bottom / TILE_SIZE);
					i <= ((GetY() - GetBaseHeight()) / TILE_SIZE) && !collision;
					i++) {
				collision = proj_collision_map[(GetX() / TILE_SIZE)
						+ i * SCENE_WIDTH];
			}
	...
	} else if (hitbox_p1.top > arrow_y
			&& hitbox_p1.bottom < arrow_y + arrow.GetHitHeight()) {
		if (hitbox_p1.left > GetX()) {
			for (i = ((GetX() + GetBaseWidth()) / TILE_SIZE);
					i <= (hitbox_p1.left / TILE_SIZE) && !collision; i++) {
				collision = proj_collision_map[i
						+ (GetY() / TILE_SIZE) * SCENE_WIDTH];
			}
			if (!collision) {
			...
			for (i = (hitbox_p1.left / TILE_SIZE);
					i <= ((GetX() - GetBaseWidth()) / TILE_SIZE) && !collision;
					i++) {
				collision = proj_collision_map[(GetX() / TILE_SIZE)
						+ i * SCENE_WIDTH];
			}
	...
	if (GetX() % TILE_SIZE != 0) {
		...
		for (i = 0; i < SCENE_WIDTH * SCENE_HEIGHT; i++)
			new_collision_map[i] = collision_map[i];
		...
		if (GetY() > p1->GetY()) {
			best_state = STATE_WALKDOWN;
		}
		if (GetX() < p1->GetX()) {
			best_state = STATE_WALKRIGHT;
		}
		if (GetX() > p1->GetX()) {
			best_state = STATE_WALKLEFT;
		}

		for (i = 0; i < occupied_tiles.size() && !illegal_move; i++)
			if (new_collision_map[occupied_tiles[i] + SCENE_WIDTH])
				illegal_move = true;
		if (!illegal_move)
			choices.push_back(STATE_WALKUP);
		else
			illegal_move = false;
		for (i = 0; i < occupied_tiles.size() && !illegal_move; i++)
			if (new_collision_map[occupied_tiles[i] - SCENE_WIDTH])
				illegal_move = true;
		if (!illegal_move)
			choices.push_back(STATE_WALKDOWN);
		else
			illegal_move = false;
		for (i = 0; i < occupied_tiles.size() && !illegal_move; i++)
			if (new_collision_map[occupied_tiles[i] + 1])
				illegal_move = true;
		if (!illegal_move)
			choices.push_back(STATE_WALKRIGHT);
		else
			illegal_move = false;
		for (i = 0; i < occupied_tiles.size() && !illegal_move; i++)
			if (new_collision_map[occupied_tiles[i] - 1])
				illegal_move = true;
		if (!illegal_move)
			choices.push_back(STATE_WALKLEFT);
		else
			illegal_move = false;

		if (choices.size()) {
			SetState(choices[rand() % choices.size()]); //assigns a random address among the possible
			for (i = 0; i < choices.size(); i++)
				if (choices[i] == best_state)
					SetState(best_state); //If one of those possible directions was the best, assign that
			Move(); //I begin to move in the assigned direction, if any, to differentiate myself from the case in which I arrive at a new tile
		}
	}
	return false;
}
