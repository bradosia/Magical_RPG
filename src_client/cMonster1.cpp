	...
	if (GetFrame() == 3 && GetDelay() == 0) {
		if (GetState() == STATE_ATTACKUP) {
			...
			if (player->GetState() != STATE_DYING
					&& Intersection(attack_area, player->GetHitBox())) {
				cActor* player1 = dynamic_cast<cActor*>(player);
				if (player1 != NULL && player1->IsDoingSkill())
					player1->TestShieldProtection(STATE_ATTACKUP, GetDamage(),
							GetX(), GetY());
				else {
					int new_health = player->GetHealth() - GetDamage();
					if (new_health <= 0)
						player->SetState(STATE_DYING);
					else if (!dynamic_cast<cActor*>(player)->IsSuperAttacking()) {
			...
			if (player->GetState() != STATE_DYING
				...
				cActor* player1 = dynamic_cast<cActor*>(player);
				if (player1 != NULL && player1->IsDoingSkill())
					player1->TestShieldProtection(STATE_ATTACKDOWN, GetDamage(),
							GetX(), GetY());
				else {
					int new_health = player->GetHealth() - GetDamage();
					if (new_health <= 0)
			...
			if (player->GetState() != STATE_DYING
				...
				cActor* player1 = dynamic_cast<cActor*>(player);
				if (player1 != NULL && player1->IsDoingSkill())
					player1->TestShieldProtection(STATE_ATTACKLEFT, GetDamage(),
							GetX(), GetY());
				else {
					int new_health = player->GetHealth() - GetDamage();
					if (new_health <= 0)
			...
			if (player->GetState() != STATE_DYING
				...
				cActor* player1 = dynamic_cast<cActor*>(player);
				if (player1 != NULL && player1->IsDoingSkill())
					player1->TestShieldProtection(STATE_ATTACKRIGHT,
							GetDamage(), GetX(), GetY());
				else {
					int new_health = player->GetHealth() - GetDamage();
					if (new_health <= 0)
...
bool cMonster1::IA(bool *collision_map, std::list<cBicho*> &enemies,
		cBicho* p1) {
	unsigned int i;
	...
	if (GetX() % TILE_SIZE != 0) {
		...
		min_dist = sqrt(
				pow(GetX() - p1->GetX(), 2.0f)
						+ pow((GetY() + TILE_SIZE) - p1->GetY(), 2.0f));
