...
bool cGame::Init(int lvl)
{
	...
	glOrtho(visible_area.left, visible_area.right, visible_area.bottom,
			visible_area.top, 3, -SCENE_HEIGHT * TILE_SIZE - 2 - 1); //ultimos 2 valores son znear y zfar, el 3 es por los overlays de barras/vidas, interfaz y menu, el -2 es por las z del terreno, rango: [-3,(SCENE_HEIGHT*TILE_SIZE)+2]
...
bool cGame::Process()
{
	...
	if (!Player1.IsAttacking() && !Player1.IsDamaged())
	{
		...
		if (doing_skill)
			Player1.SetState(Player1.GetLogicState()); //cuando una skill esta activa, el state se queda en una direccion fija por lo que en vez del atributo state, uso el logic_state de Player1 para poder calcular el movimiento y el slide
		if (keys[P1_UP])
		{
			Player1.RestrictedMoveUp(Scene.GetCollisionMap(), enemies, h1, h2,
			GAME_HEIGHT);
			stand = false;
		}
		else if (keys[P1_DOWN])
		{
			Player1.RestrictedMoveDown(Scene.GetCollisionMap(), enemies, h1, h2,
		...
		if (!doing_skill)
			input_state = Player1.GetState(); //en este caso imput_state nunca sera un state de skill

		if (keys[P1_SKILL])
		{
			if (Player1.GetSP() > 0)
				Player1.SetShieldState(input_state);
			else
				Player1.StopDoingSkillP1();
		}
		...
		if (stand)
			Player1.StopP1(); //si no hago nada muestro un estado LOOK
	...
	if (!Player2.IsAttacking() && !Player2.IsDamaged())
	{
		...
		if (stand)
			Player2.Stop(); //si no hago nada muestro un estado LOOK
	}
	...
	for (it_p = projectiles.begin(); it_p != projectiles.end();)
	{
		...
		else
		{
			...
			if (framebox.left >= visible_area.right - SCENE_Xo * 2
					...
					|| framebox.top <= visible_area.bottom) //si sale del campo de vision lo borro
			{
				delete (*it_p); //libero la memoria dinamica asignada por el new que creo al proyectil
				it_p = projectiles.erase(it_p); //borro el puntero al proyectil y actualizo el iterador
			}
	...
	for (it_e = enemies.begin(); it_e != enemies.end();)
	{
		...
		else
		{
			if (dynamic_cast<cEnemy*>(*it_e)->GetFrameTop()
					< visible_area.bottom) //si sale del campo de vision lo borro
			{
				delete (*it_e); //libero la memoria dinamica asignada por el new que creo al enemigo
				it_e = enemies.erase(it_e); //borro el puntero al enemigo y actualizo el iterador
			}
	...
	if ((Player1.IsAttacking() || Player1.IsDamaged())
			&& Player1.GetFrame() == 0 && Player1.GetDelay() == 0)
		Player1.StopP1();
	if ((Player2.IsAttacking() || Player2.IsDamaged())
	...
	if (!Player1.IsAttacking() && !Player1.IsDamaged() && p1_attacks
			&& Player1.GetCooldown() == 0)
	{
		if (Player1.GetCharge() >= CHARGE_BREAK)
		{
	...
	if (!Player1.IsAttacking())
	{
		...
		if (!Player1.IsDamaged() && !p1_defends
				&& /*Player1.GetCharge() == 0 &&*/Player1.GetSP()
						< Player1.GetMaxSP())
						...
						&& (*it_e)->GetY() <= visible_area.top) //la IA solo se ejecuta cuando no tiene tareas ya asignadas
				{
					if (dynamic_cast<cMonster1*>(*it_e) != NULL) //verificamos a que subclase pertenece el puntero
					{
						if (dynamic_cast<cMonster1*>(*it_e)->IA(
								Scene.GetCollisionMap(), enemies, &Player1,
								&Player2))
							Sound.Play(SOUND_SLASH);
					}
					if (dynamic_cast<cMonster2*>(*it_e) != NULL) //verificamos a que subclase pertenece el puntero
					{
						if (dynamic_cast<cMonster2*>(*it_e)->IA(
								...
								Scene.GetProjCollisionMap(), enemies, &Player1,
								&Player2))
	...
	//Updates
	if (Player1.GetState() == STATE_DYING || Player2.GetState() == STATE_DYING)
		state = STATE_GAMEOVER;
	else if (Player1.GetY() >= visible_area.top - TILE_SIZE
			&& Player2.GetY() >= visible_area.top - TILE_SIZE)
	{
...
void cGame::Render()
{
	...
	if (Player1.GetCharge() >= CHARGE_BREAK && Player1.GetCharge() % 4 < 2)
		Player1.Draw(Data.GetID(IMG_SUPERP1), tex_w, tex_h, run);
	else
		Player1.Draw(Data.GetID(IMG_PLAYER1), tex_w, tex_h, run);
	...
	Overlay1.DrawBars(Player1.GetMaxHealth(), Player1.GetHealth(),
			Player1.GetMaxSP(), Player1.GetSP());
