...
bool cLogic::LoadDynamicLayer(int lvl) {
	bool res;
	FILE *fd;
	char file[32];
	int i, j, n, id;
...
void cLogic::Finalize() {
}

void cLogic::ReadKeyboard(int key, int, int action, int mods) {
	if (key > 0 && key < 1000) {
		keyPress[key] = action;
...
void cLogic::ReadMouse(int button, int state, int x, int y) {
}
...
void cLogic::UpdateCamera(int h1, int h2) {
	int mean;
	if (h1 >= h2)
...
void cLogic::UpdateCamera(int h1) {
	int mean = h1;
...
void cLogic::Reshape(int w, int h) //w y h son las dimensiones de la ventana del juego
		{
...
cActor* cLogic::getActorMain() {
	return actorMain;
}

void cLogic::on(std::string event, std::function<void(cLogic*)> CB) {
	CBmap[event] = CB;
}
